...
class SearchQuery(BaseModel):
    ...
    min_score: float | None = Field(
        None, description='Minimum similarity score threshold for results (defaults to core system default of 0.6)'
    )
    mmr_lambda: float | None = Field(
        None, description='Lambda parameter for MMR reranking (0-1, defaults to core system default of 0.5)'
    )
    reranker_min_score: float | None = Field(
        None, description='Minimum score threshold for reranked results (defaults to core system default of 0.0)'
    )
    bfs_max_depth: int | None = Field(
        None, description='Maximum depth for breadth-first search (defaults to core system default of 3)'
    )
...
class SearchResults(BaseModel):
    ...
    # Search configuration and ranking metadata
    search_config_used: dict[str, Any] | None = Field(
        default=None, description='Search configuration used for the query (methods, rerankers, thresholds)'
    )
    ranking_method: str | None = Field(
        default=None, description='Primary ranking method used (rrf, mmr, cross_encoder, node_distance)'
    )
    score_normalization: str | None = Field(
        default=None, description='Method used for score normalization (none, min_max, z_score)'
    )
    max_score_possible: float | None = Field(
        default=None, description='Maximum possible score for the ranking method used'
    )
    min_score_threshold: float | None = Field(
        default=None, description='Minimum score threshold that was applied during search'
    )
...
class GetMemoryRequest(BaseModel):
    ...
    # Search configuration
    include_nodes: bool = Field(default=False, description='Whether to include nodes in search results')
    include_edges: bool = Field(default=True, description='Whether to include edges in search results')
    include_episodes: bool = Field(default=False, description='Whether to include episodes in search results')
    include_communities: bool = Field(default=False, description='Whether to include communities in search results')
